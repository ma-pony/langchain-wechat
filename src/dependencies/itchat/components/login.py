    ...
    while self.isLogging:
        ...
        while not isLoggedIn:
            ...
            if callable(qrCallback):
                qrCallback(uuid=self.uuid, status=status, qr_code=qrStorage.getvalue())
            if status == "200":
                isLoggedIn = True
            elif status == "201":
...
def push_login(core):
    ...
    if "wxuin" in cookiesDict:
        url = "{}/cgi-bin/mmwebwx-bin/webwxpushloginurl?uin={}".format(config.BASE_URL, cookiesDict["wxuin"])
...
def check_login(self, uuid=None):
    ...
    params = "loginicon=true&uuid={}&tip=1&r={}&_={}".format(
        uuid, int(-localTime / random.randint(1200, 1800)), localTime
    )
    headers = {"User-Agent": config.USER_AGENT}
    r = self.s.get(url, params=params, headers=headers)
...
def web_init(self):
    ...
    params = {
        "r": int(-time.time() / random.randint(1200, 1800)),
    ...
    self.loginInfo["synckey"] = "|".join(["{}_{}".format(item["Key"], item["Val"]) for item in dic["SyncKey"]["List"]])
...
def show_mobile_login(self):
    url = "{}/webwxstatusnotify?lang=zh_CN&pass_ticket={}".format(self.loginInfo["url"], self.loginInfo["pass_ticket"])
    data = {
        "BaseRequest": self.loginInfo["BaseRequest"],
        "Code": 3,
...
def get_msg(self):
    ...
    self.loginInfo["synckey"] = "|".join(
        ["{}_{}".format(item["Key"], item["Val"]) for item in dic["SyncCheckKey"]["List"]]
    )
